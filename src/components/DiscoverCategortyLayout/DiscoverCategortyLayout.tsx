...
const DiscoverCategortyLayout = ({ title, data, nameOfOtherButton, height, width, numOfDisplay }: DiscoverCateLayoutProps) => {
    return (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: "auto auto auto auto auto", height: "100%", width: "100%" }}>
                {data.map((i, index) => {
                    if (index < numOfDisplay) {
                        return (
                            <MusicCard title={i.title} imageAlbum={i.imageAlbum} key={i.title} />
                        )
                    } else return null
                })}
            </div>
...
export default DiscoverCategortyLayout;