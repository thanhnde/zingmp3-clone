...
;
    ...
    const handleUploadFile = async (e: any) => {
        ...
        const imageRef = refStorage(storage, `discover/mot-chut-yeu-doi/${input.id}/image/${imageFile.name}`);
        const audioRef = refStorage(storage, `discover/mot-chut-yeu-doi/${input.id}/audio/${audioFile.name}`);
        ...
        // Get old data
        const pushData = await get(refDatabase(db, `discover/mot-chut-yeu-doi`))
        ...
        await Promise.all([uploadImageTask, uploadaudioTask])
            .then(async (res) => {
                ...
                set(refDatabase(db, `discover/mot-chut-yeu-doi`), data);
            })
            .catch((error) => {
